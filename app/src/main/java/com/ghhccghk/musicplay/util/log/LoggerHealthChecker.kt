...
object LoggerHealthChecker {
    ...
    private fun checkXposedLog(): Boolean {
        try {
            val lsposedLogDirExists =
                rootExecCmd("ls -d /data/adb/lspd/log/ 2>/dev/null")?.isNotEmpty() == true

            if (lsposedLogDirExists) {
                val latestLogFile =
                    rootExecCmd("ls -t /data/adb/lspd/log/modules_*.log 2>/dev/null | head -n 1")?.trim()
                        ?: ""

                if (latestLogFile.isNotEmpty() && !latestLogFile.contains("No such file")) {
                    val grepOutput =
                        rootExecCmd("grep -i -q 'HyperCeiler' $latestLogFile && echo 'FOUND' || echo 'EMPTY'")
        ...
        val future = executor.submit<Boolean> {
            ...
            try {
                ...
                process = Runtime.getRuntime()
                    .exec(arrayOf("logcat", "-d", "-v", "threadtime", "-s", "$TAG:D"))
